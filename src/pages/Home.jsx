...
function Home() {
  ...
  const fetchStats = (itemId) => {
    ...
    fetch(
      `https://${selectedServer}.albion-online-data.com/api/v2/stats/prices/${itemId}?locations=${locations.join(
        ","
      )}&qualities=0,1,2,3,4,5&time-scale=6`
    )
      ...
      .then((data) => {
        ...
        setStats(
          uniqueCities.map((city) => {
            return {
              city: city,
              stats: data.find((stat) => stat.city === city) || {
                sell_price_min: 0,
          ...
          <div className="flex flex-col items-center">
            ...
            {stats.map((stat, index) => (
              <div key={index} className="font-mono">
                <h2 className="text-xl underline mt-5 text-center mb-2">
                  {stat.city}
                </h2>
                <p className="text-s">
          ...
          <div className="font-mono mt-5">
            <p>
              ...
              <p className="pt-5 pl-0 pr-0 ml-0 mr-0">
                ...
                @1,2,3,4 are the enchantments. Items should be searched by their
                full name.
              </p>
              <p className="pt-24">
                Note: Content is fetched every hour. If the item price is '0',
                then it's not updated at the server.
              </p>
            </p>
          </div>
        )}
